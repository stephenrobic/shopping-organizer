...
from .context_processors import user_lists, incoming_friend_requests, \
    outgoing_friend_requests, current_friends, shared_lists
...
def home(request):
    context = user_lists(request) | incoming_friend_requests(request) \
              | outgoing_friend_requests(request) | current_friends(request) | shared_lists(request)
...
def add_friend(request, request_id=None, friend_name=None):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get("detail"):
            current_list_id = int(request.POST.get("detail"))
            return HttpResponseRedirect('/list_details/%i' % current_list_id)
        else:
            return HttpResponseRedirect('/')
...
def create_list(request):
    ...
    context = incoming_friend_requests(request) | outgoing_friend_requests(request) \
              | current_friends(request) | shared_lists(request)
...
def list_details(request, list_id):
    ...
    if request.method == 'POST':
        if current_list in request.user.shoppinglist.all() or current_list in request.user.sharedfriends.all():
            ...
            if request.POST.get("edit_item"):
                ...
                current_list.save()
                return HttpResponseRedirect('/list_details/%i' % list_id)
            if request.POST.get("share_list"):
                friend_name = request.POST.get("share_list")
                friend_obj = User.objects.get(username=friend_name)
                current_list.share_list(friend_obj)
                return HttpResponseRedirect('/list_details/%i' % list_id)
        else:
            return HttpResponse("YOU DON'T HAVE PERMISSION TO EDIT THIS LIST!")
    elif current_list in request.user.shoppinglist.all() or current_list in request.user.sharedfriends.all():
        unauthorized_friends = User.objects.filter(friends__user=request.user).exclude(sharedfriends__user__is_active=True)
        authorized_friends = User.objects.filter(sharedfriends__user__username=current_list.user)
        context = {'current_list': current_list} | user_lists(request) | incoming_friend_requests(request) \
                  | outgoing_friend_requests(request) | current_friends(request) | shared_lists(request)\
                  | {'unauthorized_friends': unauthorized_friends} | {'authorized_friends': authorized_friends}\
                  | {'page': 'detail'}
